...
class FormaGeometrica:
    ...
    def get_base(self):
        """Metodo getter para atributo __base"""
        return self.__base
    
    def get_altura(self):
        """Metodo getter para o atributo __altura"""
        return self.__altura
    
    def get_tipo(self):
        """Metodo getter para o atributo __tip  mnnn mn mn mn m,.no"""
        return self.__tipo
    
    def calc_area(self):
        if self.__tipo == "R":           #Retangulo
            return self.__base * self.__altura
        elif self.__tipo == "T":         #Triangulo
            return self.__base * self.__base / 2
        else:                            #Elipse/Circulo
            return (self.__base / 2) * (self.__altura / 2) * pi
        
    def __str__(self):
        """Provê uma reprsentação do objeto como string"""
        return f"[FormaGeometrica], base: {self.__base}, altura: {self.__altura}, tipo: {self.__tipo}, area: {self.calc_area()}"
...
print(f"Dados da forma1: base = {forma1.get_base()}, altura = {forma1.get_altura()}, tipo = {forma1.get_tipo()}, area = {forma1.calc_area()}")
