def busca_sequencial(lista, val):
    """
    Função que realiza uma busca sequencial em uma lista, procurando por val. Se o val for encontrando, 
    retorna a posição de val na lista.
    Caso, contrario, retorna o valor convencional -1.
    """
    #Percorre a lista do inicio ao fim, usando range() e len() (é necessário ter acesso as posições dos elementos)
    for pos in range(len(lista)):
        #Encontra val; retorna a posição onde foi encontrado
        if val == lista[pos]: return pos

    #<-- CUIDADO COM A IDENTAÇÃO AQUI
    #Percorreu toda a lista e não encontrou; retorna -1
    return -1
...
resultado4 = busca_sequencial(nomes, "GLICINEIA GARCIA")
print(f"GLICINEIA GARCIA encontrada na posição {resultado4}")
